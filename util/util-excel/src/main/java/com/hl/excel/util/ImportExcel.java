 ...
 * 2.读取数据
 * List<PhoneModelExcelTest> list =  ImportExcel.readXls("C://test.xlsx",ImportExcel.getMap(keyValue),"com.lkx.excel.PhoneModelExcelTest");
 *
 * @author Hailin
 */
public class ImportExcel implements Serializable {
    ...
    public static <T> List<T> readExcel(String fileName, InputStream inputStream, Map<String, String> map,
        ...
        for (int sheetNum = 0; sheetNum < 1; sheetNum++) {
            ...
            for (int rowNum = 0; rowNum <= hssfSheet.getLastRowNum(); rowNum++) {
                ...
                if (rowNum_x == -1) {
                    //判断指定行是否为空
                    rowNumIndex = rowNumIndex - 1;
                    Row hssfRow = hssfSheet.getRow(rowNumIndex);
                    if (hssfRow == null) {
                        throw new RuntimeException("表头为空，请检查！");
                    }
                    //设置当前行为指定行
                    rowNum = rowNumIndex;
                }
    ...
    public static void exportExcel(String titleText, OutputStream out, Map<String, String> map, List<?> list, String classPath, HSSFCellStyle titleStyle, HSSFCellStyle headStyle, HSSFCellStyle dataStyle) throws Exception {
        ...
        try {
            ...
            e.printStackTrace();
            throw new Exception("IOException!导出失败！");
        }
